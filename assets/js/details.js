...
(async function(){
  ...
  try {
    ...
    $('#title').textContent=item.title || '—'; $('#arabicTitle').textContent=item.arabic_title || '';
    $('#shortDescription').textContent=item.short_description || ''; $('#description').textContent=item.description || 'لا توجد تفاصيل إضافية.';
    $('#views').textContent=fmt((item.views||0)+(!Zero9DB.isDemo?1:0)); $('#downloads').textContent=fmt(item.downloads); $('#version').textContent=item.version || '—';
    ...
    $('#gallery').innerHTML = shots.length ? shots.map((src,i)=>`<button data-src="${esc(src)}" aria-label="فتح الصورة ${i+1}"><img src="${esc(src)}" alt="صورة من التعريب ${i+1}" loading="lazy"></button>`).join('') : '<div class="empty">لا توجد صور إضافية لهذا التعريب.</div>';
    ...
    const close=()=>$('#lightbox').classList.remove('open'); $('#lightboxClose').onclick=close; $('#lightbox').addEventListener('click',e=>{if(e.target===$('#lightbox'))close();});
    if (!Zero9DB.isDemo) {
      try {
    ...
    $('#shareBtn').onclick=async()=>{
      ...
      try{if(navigator.share)await navigator.share(shareData);else{await navigator.clipboard.writeText(location.href);toast('تم نسخ رابط التعريب.');}}catch(e){if(e.name!=='AbortError')toast('تعذر تنفيذ المشاركة.',true);}
    };
  } catch(e) { $('#detailsRoot').innerHTML=`<section class="section"><div class="container"><div class="empty">تعذر تحميل التفاصيل: ${esc(e.message)}</div></div></section>`; }
})();
